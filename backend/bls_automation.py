...
class BLSAutomation:
    ...
    async def handle_captcha_grid(self, page: Page, target_number: str) -> bool:
        """Handle grid-based captcha system"""
        try:
            await self.log(LogLevel.INFO, f"Processing grid captcha for target: {target_number}", step="CAPTCHA_GRID")
            
            # Look for captcha grid tiles
            tiles = await page.query_selector_all('img[src*="captcha"], .captcha-tile, [data-tile]')
            
            if not tiles:
                await self.log(LogLevel.WARNING, "No captcha tiles found", step="CAPTCHA_GRID")
                return False
            ...
            for i, tile in enumerate(tiles):
                try:
                    # Get image source
                    img_src = await tile.get_attribute('src')
                    if not img_src:
                        continue
                    ...
                    if img_src.startswith('data:image'):
                        ...
                        # Navigate to image and get base64
                        try:
                            response = await page.goto(img_src)
                    ...
                    await self.log(LogLevel.WARNING, f"Error processing tile {i}: {str(e)}", step="CAPTCHA_GRID")
                    continue
            
            if not tile_data:
                await self.log(LogLevel.ERROR, "No valid tile data collected", step="CAPTCHA_GRID")
                return False
            ...
            if response.status_code == 200:
                ...
                await self.log(LogLevel.INFO, f"OCR found {len(matching_indices)} matching tiles", 
                             details={"indices": matching_indices}, step="CAPTCHA_GRID")
                ...
                for idx in matching_indices:
                    if idx < len(tiles):
                        try:
                            ...
                            await self.log(LogLevel.WARNING, f"Error clicking tile {idx}: {str(e)}", step="CAPTCHA_GRID")
                
                await self.log(LogLevel.SUCCESS, f"Clicked {len(matching_indices)} captcha tiles", step="CAPTCHA_GRID")
                return True
            else:
                await self.log(LogLevel.ERROR, f"OCR API failed with status {response.status_code}", step="CAPTCHA_GRID")
                return False
                
        except Exception as e:
            await self.log(LogLevel.ERROR, f"Grid captcha handling failed: {str(e)}", step="CAPTCHA_GRID")
            return False

    async def handle_captcha_login(self) -> bool:
        """Handle captcha during login"""
        try:
            # Look for captcha grid first (priority)
            captcha_grid = await self.page.query_selector_all('img[src*="captcha"], .captcha-tile, [data-tile]')
            
            if captcha_grid and len(captcha_grid) > 1:
                # This is a grid-based captcha
                await self.log(LogLevel.INFO, f"Found grid captcha with {len(captcha_grid)} tiles", step="CAPTCHA_LOGIN")
                ...
                return await self.handle_captcha_grid(self.page, target_number)
            
            # Look for single captcha image
    ...
    async def handle_appointment_captcha(self) -> bool:
        ...
        try:
            await self.log(LogLevel.INFO, "Handling appointment captcha", step="APPOINTMENT_CAPTCHA")
            ...
            if captcha_grid and len(captcha_grid) > 1:
                # This is a grid-based captcha
                await self.log(LogLevel.INFO, f"Found appointment grid captcha with {len(captcha_grid)} tiles", step="APPOINTMENT_CAPTCHA")
                
                # Extract target number from page
                target_text = await self.page.text_content('body')
                target_match = re.search(r'select.*?(\d+)', target_text, re.IGNORECASE)
                if not target_match:
                    await self.log(LogLevel.ERROR, "Could not find target number in appointment captcha", step="APPOINTMENT_CAPTCHA")
                    return False
                
                target_number = target_match.group(1)
                return await self.handle_captcha_grid(self.page, target_number)
            
            # Fallback to single captcha handling
            return await self.handle_captcha_login()
            
        except Exception as e:
            await self.log(LogLevel.ERROR, f"Appointment captcha handling failed: {str(e)}", step="APPOINTMENT_CAPTCHA")
            return False

    async def parse_available_slots(self) -> List[AppointmentSlot]:
        """Parse available appointment slots from the BLS page"""
        try:
            slots = []
            ...
            # Look for appointment slots in various formats
            slot_selectors = [
                '.appointment-slot',
                '.slot-item', 
                '.available-slot',
                'tr:has-text("Available")',
            ...
            for selector in slot_selectors:
                try:
                    ...
                    if elements:
                        ...
                        await self.log(LogLevel.INFO, f"Found {len(elements)} potential slots with selector: {selector}", step="SLOT_PARSING")
                except:
                    continue
            
            # If no structured slots found, look for any availability indicators
            if not slot_elements:
                # Look for any text indicating availability
                page_text = await self.page.text_content('body')
                availability_keywords = ['available', 'book now', 'select date', 'appointment slot', 'open slot']
                
                for keyword in availability_keywords:
                    if keyword.lower() in page_text.lower():
                        await self.log(LogLevel.INFO, f"Found availability keyword: {keyword}", step="SLOT_PARSING")
                        
                        # Create a generic slot since we found availability
            ...
            # Process found slot elements
            for element in slot_elements:
                try:
                    text = await element.text_content() or ""
                    
                    # Skip if no availability indication
                    if not any(keyword in text.lower() for keyword in ['available', 'book', 'select', 'open']):
                        continue
                    
                    # Extract date and time (this would need customization based on actual page structure)
                    ...
                    # Try to extract visa type and category
                    visa_type = "Spain Visa"
                    visa_category = "General"
                    
                    # Look for visa type indicators
                    if "tourism" in text.lower():
                    ...
                    await self.log(LogLevel.WARNING, f"Error parsing slot element: {str(e)}", step="SLOT_PARSING")
                    continue
            
            # Log results
            if slots:
                await self.log(LogLevel.SUCCESS, f"Successfully parsed {len(slots)} appointment slots", 
                             details={"slots": [s.dict() for s in slots]}, step="SLOT_PARSING")
            else:
                await self.log(LogLevel.INFO, "No appointment slots found in current page", step="SLOT_PARSING")
                
                # Take a screenshot for debugging
                try:
                    screenshot = await self.page.screenshot()
                    await self.log(LogLevel.INFO, "Screenshot taken for debugging", 
                                 details={"screenshot_size": len(screenshot)}, step="SLOT_PARSING")
                except:
                    pass
            ...
            await self.log(LogLevel.ERROR, f"Failed to parse appointment slots: {str(e)}", step="SLOT_PARSING")
            return []
